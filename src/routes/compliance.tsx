...
function CompliancePage() {
  ...
  const grouped = useMemo(() => {
    ...
    const map = new Map<string, {
      ...
      due_date: string;
      clients: any[];
      status_counts: { pending: number; filed: number; overdue: number };
    }>();

    items
      .filter((item) => item.status !== "client_deleted" && item.status !== "filed")
      .forEach((item) => {
      const complianceName = item.compliance_name ?? item.compliance_type ?? "—";
      const dueDate = item.due_date ?? "";
      const key = `${complianceName}_${dueDate}`;
      if (!map.has(key)) {
        map.set(key, {
          compliance_name: complianceName,
          due_date: dueDate,
          clients: [],
          status_counts: { pending: 0, filed: 0, overdue: 0 },
        });
      }
      ...
      if (due < today) {
        group.status_counts.overdue++;
      } else {
        group.status_counts.pending++;
      }
      });

    return Array.from(map.values())
      .filter((group) => group.clients.filter(
  ...
  const markAllFiled = async (group: (typeof grouped)[number]) => {
    const pendingIds = group.clients
      .filter((client: any) => client.status !== "filed")
      ...
      {!isLoading && grouped.length > 0 && (
        <div className="space-y-5">
          {grouped.map((group, index) => {
            ...
            const month = groupDate
              ? groupDate.toLocaleDateString("en-IN", { month: "long", year: "numeric" }).toUpperCase()
              : "UNDATED";
            const previousDate = index > 0 && grouped[index - 1].due_date
              ? new Date(`${grouped[index - 1].due_date}T00:00:00`)
              : null;
            const previousMonth = previousDate
              ? previousDate.toLocaleDateString("en-IN", { month: "long", year: "numeric" }).toUpperCase()
              : "UNDATED";
            const diff = groupDate
              ? Math.ceil((groupDate.getTime() - startOfDay(new Date()).getTime()) / (1000 * 60 * 60 * 24))
              : null;
