...
function RequestModal({
  ...
  const removeDocName = (i: number) => {
    setDocNames(docNames.filter((_, idx) => idx !== i));
  };
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              disabled={!clientId}
              className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-50 disabled:text-slate-400"
            >
              <option value="">
                {clientId ? "None" : "Select a client first"}
              </option>
