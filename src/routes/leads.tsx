        ...
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            ...
            {!isLoading && filtered?.length === 0 && (
              <tr><td colSpan={9} className="px-5 py-8 text-center text-muted-foreground">No leads found.</td></tr>
            )}
            {filtered?.map((l) => (
              <tr key={l.id} className="hover:bg-muted">
                <td className="px-5 py-3 font-medium text-foreground">{l.name}</td>
                <td className="px-5 py-3 text-foreground">{l.phone ?? "—"}</td>
                <td className="px-5 py-3 text-foreground">{l.email ?? "—"}</td>
                <td className="px-5 py-3 text-foreground">{l.source ?? "—"}</td>
                <td className="px-5 py-3 text-foreground">{l.requirement ?? "—"}</td>
                <td className="px-5 py-3 text-foreground">{l.urgency ?? "—"}</td>
                <td className="px-5 py-3">
                  {l.qualification_score ? (
                    <span className={`px-2 py-1 rounded-md text-xs font-medium ${scoreColors[l.qualification_score] ?? "bg-muted text-foreground"}`}>
                      {l.qualification_score}
                    </span>
                  ) : "—"}
                </td>
                <td className="px-5 py-3">
                  <span className={`px-2 py-1 rounded-md text-xs font-medium ${statusColors[l.status] ?? "bg-muted text-foreground"}`}>
                    {l.status}
                  </span>
                </td>
                <td className="px-5 py-3">
