...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside className={`${open ? "block" : "hidden"} md:block bg-sidebar text-sidebar-foreground w-full md:w-64 md:min-h-screen md:fixed md:top-0 md:left-0 md:flex md:flex-col md:border-r md:border-sidebar-border`}>
        {/* Desktop header */}
        <div className="hidden md:flex items-center gap-3 px-5 h-16 border-b border-sidebar-border">
          ...
          <div className="leading-tight min-w-0">
            <div className="font-display font-semibold text-sidebar-foreground text-[15px] leading-tight truncate">{displayFirmName}</div>
        ...
        <nav className="p-3 space-y-0.5 flex-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              <NavLink key={item.to} to={item.to} label={item.label} icon={item.icon} active={active} onClick={() => setOpen(false)} />
            );
          })}
        </nav>
